shader_type spatial;
render_mode diffuse_lambert, vertex_lighting, cull_back, shadows_disabled, depth_draw_opaque, unshaded;
...
uniform bool uv_pan_wavey = true;
uniform float wave_strength = 0.3;
uniform vec2 wave_size = vec2(4.0);
...
void vertex()
{
	float pan_offset = mix(TIME, sin(TIME), float(uv_pan_wavey));
	UV = UV * uv_scale + uv_offset;
	UV += uv_pan_velocity * pan_offset;
	
	float t = TIME * 0.5;
	
	VERTEX.y += cos((VERTEX.x + t) * wave_size.x) * sin((VERTEX.z + t) * wave_size.y) * wave_strength;
...
void fragment() {
	...
	ALBEDO = albedo_tex.rgb * COLOR.rgb * albedo_color.rgb;
}